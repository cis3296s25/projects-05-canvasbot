...
class stud_util(commands.Cog):
    ...
    async def ai_announcements(self, interaction: Interaction):
        ...
        print("This is before the announcement data is obtained.")
    ...
    async def automatic_announcements(self, interaction: Interaction):
        ...
        if pick == 1:
            print("1")
            await interaction.followup.send("You will now recieve messages with announcements from the last 7 days.");
            self.send_announcements_daily.start()
        elif pick == 0:
            print("0")
            await interaction.followup.send("You will not recieve any messages with announcements.");
        else:
            print("invalid number")
            await interaction.followup.send("Invalid choice. No announcement messages will be sent.");
